...
void drawMcClosureTest(int xNorm = 1) { 
  ...
  for (int ipt=1 ; ipt<=nPtBin ; ipt++) {  
    for (int icoll=4 ; icoll<6 ; icoll++) {  
      ...
      histFile[icoll][ipt] = new TFile(Form("ffFiles/photonTrackCorr_%s_output_photonPtThr%d_to_%d_jetPtThr30_20130820.root",sampleName.Data(), (int)ptBin[ipt-1], (int)ptBin[ipt]) );
      cout << " Reading file : " << Form("ffFiles/photonTrackCorr_%s_output_photonPtThr%d_to_%d_jetPtThr30_20130820.root",sampleName.Data(), (int)ptBin[ipt-1], (int)ptBin[ipt]) << endl;
