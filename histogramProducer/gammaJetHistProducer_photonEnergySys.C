...
void gammaJetHistProducer_photonEnergySys(sampleType collision = kPADATA, float photonPtThr=60, float photonPtThrUp=9999, float jetPtThr=30, int icent =1, float scaleFactor = -0.015) {
  ...
  TCut ptPhoCut  = Form("(photonEt*(1+%f)) >%.1f && (photonEt*(1+%f))<%.1f", (float)scaleFactor, (float)photonPtThr, (float)scaleFactor, (float)photonPtThrUp  );
  TCut caloIso;
  ...
  tgj->Draw2(gSpec->hPtPhoCand,  Form("(photonEt*(1+%f))", (float)scaleFactor), phoCandCut, "")  ;
  tgj->Draw2(gSpec->hPtPhoDecay, Form("(photonEt*(1+%f))", (float)scaleFactor), phoDecayCut, "") ;
