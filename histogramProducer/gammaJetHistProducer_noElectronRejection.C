...
void gammaJetHistProducer_noElectronRejection(sampleType collision = kPADATA, float photonPtThr=60, float photonPtThrUp=9999, float jetPtThr=30, int icent =1) {
  TH1::SetDefaultSumw2();
  ...
  TString fname = "";
  if ( collision == kHIDATA)      fname = fnameHIDATA_noElectronRejection; //_jetResCorrected; 
  else if ( collision == kPADATA) fname = fnamePADATA_noElectronRejection;
  else if ( collision == kPPDATA) {
    if ( icent == 7 ) fname = fnamePPDATA_noElectronRejection;
