...
void gammaJetHistProducer_leadingJet(sampleType collision = kPADATA, float photonPtThr=60, float photonPtThrUp=9999, float jetPtThr=30, int icent =1) {
  TH1::SetDefaultSumw2();
  ...
  TCut jetCut     =  Form("(abs(pt - lJetPt) < 0.1) && abs(eta)<%f && pt>%f", (float)cutjetEta, (float)jetPtThr );
  ...
  TCut jetCut2   = Form("(abs(pt-lJetPt)<0.1) && abs(dphi)>%f && pt>%f", (float)awayRange, (float)jetPtThr ) ;
