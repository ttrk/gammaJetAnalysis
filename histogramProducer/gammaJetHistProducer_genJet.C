...
#include "../CutAndBinCollection2012_genJetcollection.h"
...
void gammaJetHistProducer_genJet(sampleType collision = kPADATA, float photonPtThr=60, float photonPtThrUp=9999, float jetPtThr=30, int icent =1) {
  TH1::SetDefaultSumw2();
  ...
  TFile outf = TFile(Form("ffFilesGenJet/%s",outName.Data()),"update");
			 ...
			 TH1D* hTemplate,  TString outfName)
{
  ...
  TFile outf = TFile(Form("ffFilesGenJet/%s",outfName.Data()),"update");
