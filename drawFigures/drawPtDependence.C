...
void drawPtDependence( bool saveFigures=false) {
  ...
  for (int ipt=1 ; ipt<=nPtBin ; ipt++) {
    for (int icoll=0 ; icoll<6 ; icoll++) {
      ...
      char* fname =  Form("../histogramProducer/ffFiles/photonTrackCorr_%s_output_photonPtThr%d_to_%d_jetPtThr30_20130908.root",sampleName.Data(), (int)ptBin[ipt-1], (int)ptBin[ipt]);
