...
void drawIaaMC( bool saveFigures=true) {
	...
	char* fname =  Form("ffFiles/systematics_photonIso/photonTrackCorr_%s_output_photonPtThr%d_to_%d_jetPtThr30_20131009_genOrReco1.root",sampleName.Data(), (int)ptBin[ipt-1], 9999);
    ...
    for ( int icent = 1; icent <= nCentBinHI ; icent++ ) {
      for (int ipt=1 ; ipt<=nPtBin ; ipt++) {
	hIaa[icent][ipt] = (TH1D*)hJetPt[kHIMC][icent][ipt]->Clone(Form("iaa_%s",hJetPt[kHIMC][icent][ipt]->GetName()));
	hIaa[icent][ipt]->Divide(hJetPt[kPPMC][7][ipt]);
      }
    }
	...
	hJetPt[kPPMC][7][ipt]->Draw("same");
	    ...
	    hJetPt[kHIMC][icent][ipt]->Draw("same");
        }
	...
	*/
    ...
    easyLeg(l2,"2.76TeV");
    l2->AddEntry(hJetPt[kPPMC][7][1],"pp", "p");
    l2->AddEntry(hJetPt[kHIMC][2][1],"PbPb 30-100%", "p");
    l2->AddEntry(hJetPt[kHIMC][1][1],"PbPb 0-30%", "p");
    l2->Draw();
    ...
    for ( int ipt = 1 ; ipt<=nPtBin  ; ipt++) {
      hJetPt[kPPMC][7][ipt]->SetName(Form("dNdJetPt_forIaa_pp_ptBin%dAndHigher",ipt));
      hJetPt[kPPMC][7][ipt]->Write();

      for ( int icent = 1; icent <= nCentBinHI ; icent++ ) {
        hJetPt[kHIMC][icent][ipt]->SetName(Form("dNdJetPt_forIaa_pbpb_centralityBin%d_ptBin%dAndHigher",icent,ipt));
