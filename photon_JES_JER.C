...
const Int_t iFit=0;
const int knpx=2000;
const float fitmin=0.00;
const float fitmax=5.00;
...
void photon_JES_JER(bool savePlots)
{
  ...
  for(int i = 0; i < nptbins; i++)
  {
    ...
    TString hName = Form("reco_over_gen_pt_%d",(int)ptbins[i]);
    TH1F *ratio = new TH1F(hName,Form(";reco/gen pt %d",(int)ptbins[i]),200,fitmin,fitmax);
